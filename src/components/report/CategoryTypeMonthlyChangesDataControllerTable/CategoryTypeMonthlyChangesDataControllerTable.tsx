...
import { DateType, useCategoryTypeMonthlyChangesQuery } from '@/api/graphql'
import {
  Card,
...
export const CategoryTypeMonthlyChangesDataControllerTable: React.FC = () => {
  ...
  const { data } = useCategoryTypeMonthlyChangesQuery({
    variables: {
      input: {
    ...
    <div className="flex flex-col space-y-2">
      ...
      <Card>
        <div className="flex flex-col space-y-2">
          ...
          <MonthlyChangesTable data={data?.categoryTypeMonthlyChanges ?? []} />
        </div>
      </Card>
    </div>
  )
